      ...
      <div className="relative flex h-full aspect-[9/19] max-w-full flex-col overflow-hidden rounded-[18px] border border-white/15 bg-carbon p-[3px] shadow-[0_18px_50px_oklch(0_0_0/0.6)]">
        <div className="relative flex h-full w-full flex-col overflow-hidden rounded-[15px] bg-[oklch(0.12_0.02_260)] text-[6px] leading-tight text-foreground">
          ...
          <div className="flex shrink-0 items-center gap-1.5 px-2.5 py-1.5">
            ...
            <span className="min-w-0 flex-1 truncate text-[6.5px] font-bold tracking-tight">
              {title}
            </span>
            <span className="relative h-3 w-3 shrink-0 rounded-full bg-white/10">
              {badge && (
            ...
            <span className="h-3 w-3 shrink-0 rounded-full bg-gradient-to-br from-electric to-racing" />
          </div>

          {/* search */}
          <div className="shrink-0 px-2.5">
            <div className="flex items-center gap-1 truncate rounded-full border border-white/10 bg-white/[0.05] px-2 py-[3px] text-[4.5px] text-muted-foreground">
              <span className="h-[5px] w-[5px] shrink-0 rounded-full border border-white/40" />
              <span className="truncate">Search sponsors, teams, events…</span>
            </div>
          </div>

          {/* body */}
          <div className="flex min-h-0 flex-1 flex-col gap-1.5 overflow-hidden p-2.5">{children}</div>

          {/* tab bar */}
          <div className="flex shrink-0 items-center justify-around border-t border-white/8 bg-white/[0.03] px-1 pb-1.5 pt-1">
            {TABS.map((t) => (
              <span
                key={t.label}
                className={`flex flex-col items-center gap-[1px] text-[4px] ${
                  t.label === active ? "text-electric" : "text-muted-foreground"
                }`}
              >
                <span className="text-[6px] leading-none">{t.dot}</span>
                {t.label}
              </span>
            ))}
          </div>
...
function Ring({ value, label }: { value: number; label: string }) {
  ...
  return (
    <div className="relative grid h-[26px] w-[26px] shrink-0 place-items-center">
      ...
      <span className="text-[5px] font-bold">{label}</span>
    </div>
  );
}
...
function Chip({ text, tone = "muted" }: { text: string; tone?: "muted" | "electric" | "racing" }) {
  ...
  return (
    <span className={`whitespace-nowrap rounded-full border px-1.5 py-[2px] text-[4.5px] ${cls}`}>
      {text}
    </span>
  );
}
...
function Dashboard() {
  return (
    <Phone title="Dashboard" active="Home" badge="3">
      ...
      {[
        ...
        >
          <span className="grid h-4 w-4 shrink-0 place-items-center rounded-[4px] bg-gradient-to-br from-electric/70 to-racing/70 text-[5px] font-black">
            {r.n[0]}
          </span>
          <span className="min-w-0 flex-1">
            <span className="block truncate text-[6px] font-semibold">{r.n}</span>
            <span className="block truncate text-[4.5px] text-muted-foreground">{r.t}</span>
          </span>
...
function Recommendations() {
  return (
    <Phone title="Recommendations" active="Match">
      ...
      {[
        ...
        <div key={r.n} className="rounded-[7px] border border-white/10 bg-white/[0.035] p-1.5">
          <div className="flex items-center gap-1.5">
            <span className="grid h-4 w-4 shrink-0 place-items-center rounded-[4px] bg-white/10 text-[5px] font-black">
              {r.n[0]}
            </span>
            <span className="min-w-0 flex-1">
              <span className="block truncate text-[6px] font-semibold">{r.n}</span>
              <span className="block truncate text-[4.5px] text-muted-foreground">
                {r.i} · {r.b}
              </span>
            </span>
            <span className="shrink-0 whitespace-nowrap rounded-full border border-electric/40 bg-electric/12 px-1.5 py-[2px] text-[4.5px] font-bold text-electric">
              {r.c}%
            </span>
          </div>
          ...
          <div className="mt-1 flex items-center gap-1">
            <span className="min-w-0 flex-1 truncate text-[4.5px] text-silver">✦ {r.f}</span>
            <span className="shrink-0 whitespace-nowrap rounded-full bg-electric px-1.5 py-[2px] text-[4.5px] font-bold text-carbon">
              Connect
            </span>
          </div>
        </div>
      ...
      <p className="mt-auto text-center text-[4.5px] text-muted-foreground">12 more matches below</p>
    </Phone>
  );
}
...
function Profiles() {
  return (
    <Phone title="Sponsor Profile" active="Match">
      <div className="rounded-[7px] border border-white/10 bg-gradient-to-br from-electric/10 to-racing/10 p-1.5">
        <div className="flex items-center gap-1.5">
          <span className="grid h-5 w-5 shrink-0 place-items-center rounded-[6px] bg-gradient-to-br from-electric to-racing text-[7px] font-black">
            V
          </span>
          <span className="min-w-0 flex-1">
            <span className="block truncate text-[6.5px] font-bold">Velocity Energy</span>
            <span className="block truncate text-[4.5px] text-muted-foreground">
              Beverage · Global · Verified ✓
            </span>
          </span>
          <span className="shrink-0 whitespace-nowrap rounded-full border border-electric/40 bg-electric/12 px-1.5 py-[2px] text-[4.5px] text-electric">
            96%
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-1">
        <Stat label="Budget" value="₹1.2M" delta="/season" />
        <Stat label="Audience" value="8.4M" delta="18–34" />
        <Stat label="Deals" value="17" delta="since '19" />
      </div>
      ...
      <div className="mt-auto flex gap-1">
        <span className="flex-1 whitespace-nowrap rounded-full bg-electric py-[4px] text-center text-[5px] font-bold text-carbon">
          Send Proposal
        </span>
        <span className="whitespace-nowrap rounded-full border border-white/15 px-2 py-[4px] text-center text-[5px]">
          Save
        </span>
      </div>
    </Phone>
...
function Analytics() {
  return (
    <Phone title="Team Analytics" active="Teams">
      ...
      <div className="rounded-[6px] border border-white/10 bg-white/[0.035] p-1.5">
        <Label>Race-day engagement</Label>
      ...
      <div className="rounded-[6px] border border-white/10 bg-white/[0.035] p-1.5">
        <Label>Reach trend</Label>
        <div className="mt-1 h-[22px]">
          <Spark points="0,26 14,22 28,24 42,15 56,17 70,9 84,11 100,3" />
        </div>
      </div>

      <div className="mt-auto rounded-[6px] border border-white/10 bg-white/[0.035] p-1.5">
        ...
        {[
          ...
          <div key={d.l} className="mt-1 flex items-center gap-1">
            <span className="w-[16px] shrink-0 text-[4.5px] text-muted-foreground">{d.l}</span>
            <span className="h-[4px] min-w-0 flex-1 rounded-full bg-white/10">
              <span
                className="block h-full rounded-full bg-gradient-to-r from-electric to-racing"
                style={{ width: `${d.v * 2.2}%` }}
              />
            </span>
            <span className="w-[12px] shrink-0 text-right text-[4.5px]">{d.v}%</span>
          </div>
        ))}
      </div>
    </Phone>
    ...
    <Phone title="Workspace" active="Chat" badge="5">
      ...
      {[
        ...
        <div
          ...
          className="flex items-center gap-1.5 rounded-[6px] border border-white/10 bg-white/[0.035] p-1.5"
        >
          <span className="grid h-4 w-4 shrink-0 place-items-center rounded-full bg-white/10 text-[5px] font-black">
            {c.n[0]}
          </span>
          <span className="min-w-0 flex-1">
          ...
          <span className="shrink-0 text-[4px] text-muted-foreground">{c.t}</span>
          {c.u > 0 && (
            <span className="grid h-[8px] w-[8px] shrink-0 place-items-center rounded-full bg-racing text-[4px] font-bold">
              {c.u}
            </span>
          )}
        </div>
      ...
      <div className="rounded-[6px] border border-white/10 bg-white/[0.035] p-1.5">
        ...
        <div className="mt-1 flex items-center gap-1">
          <span className="grid h-4 w-3 shrink-0 place-items-center rounded-[2px] bg-electric/20 text-[4px] text-electric">
            PDF
          </span>
          <span className="min-w-0 flex-1 truncate text-[5px]">SponsorSphere_Tier2_v3.pdf</span>
          <span className="shrink-0 whitespace-nowrap rounded-full border border-electric/40 px-1 py-[1px] text-[4px] text-electric">
            In review
          </span>
        </div>
        <div className="mt-1 h-[3px] w-full rounded-full bg-white/10">
      ...
      <div className="rounded-[6px] border border-racing/25 bg-racing/[0.07] p-1.5">
        <Label tone="racing">Meeting schedule</Label>
        {[
          { t: "4:30 PM", n: "Velocity · Kickoff" },
          { t: "Tomorrow", n: "Apex · Negotiation" },
        ].map((m) => (
          <div key={m.t} className="mt-[3px] flex items-center gap-1">
            <span className="h-[4px] w-[4px] shrink-0 rounded-full bg-racing" />
            <span className="shrink-0 text-[4.5px] font-semibold">{m.t}</span>
            <span className="truncate text-[4.5px] text-muted-foreground">{m.n}</span>
          </div>
        ))}
      </div>

      <div className="mt-auto flex items-center gap-1 rounded-full border border-white/10 bg-white/[0.05] px-2 py-[3px] text-[4.5px] text-muted-foreground">
        <span className="min-w-0 flex-1 truncate">Message Velocity Energy…</span>
        <span className="shrink-0 whitespace-nowrap rounded-full bg-electric px-1.5 py-[1px] text-[4px] font-bold text-carbon">
          Send
        </span>
      </div>
    </Phone>
  ...
  return (
    <Phone title="AI Insights" active="You">
      <div className="rounded-[7px] border border-electric/30 bg-electric/[0.08] p-1.5">
        <Label tone="electric">AI summary</Label>
        <p className="mt-[3px] text-[4.5px] leading-[1.5] text-silver">
        ...
        <div className="mt-1 flex items-center gap-1">
          ...
          <span className="truncate text-[4.5px] text-muted-foreground">Confidence score</span>
        </div>
      </div>
      ...
      <div className="rounded-[6px] border border-white/10 bg-white/[0.035] p-1.5">
        ...
        <div className="mt-1 grid grid-cols-8 gap-[2px]">
          {[
            18, 42, 66, 88, 34, 72, 95, 25, 55, 30, 80, 60, 44, 90, 20, 70,
          ].map((v, i) => (
            <span
              key={i}
              ...
              style={{
                background:
                  v > 75 ? "var(--racing)" : v > 50 ? "var(--electric)" : "oklch(1 0 0 / 0.12)",
      ...
      <div className="rounded-[6px] border border-white/10 bg-white/[0.035] p-1.5">
        <Label>Predicted deal value</Label>
        <div className="mt-1 h-[22px]">
          <Spark points="0,24 16,20 32,22 48,13 64,15 80,7 100,2" stroke="var(--racing)" />
        </div>
        <p className="mt-[2px] truncate text-[4.5px] text-silver">₹6.1M by season end (+27%)</p>
      </div>

      <div className="mt-auto flex items-center gap-1.5 rounded-[6px] border border-white/10 bg-white/[0.035] p-1.5">
        <span className="shrink-0 text-[7px] text-electric">✦</span>
        <span className="min-w-0 flex-1">
          <span className="block truncate text-[5.5px] font-semibold">Re-engage NovaTech Labs</span>
          <span className="block truncate text-[4.5px] text-muted-foreground">
            Dormant 21 days · 68% revive odds
          </span>
        </span>
        <span className="shrink-0 whitespace-nowrap rounded-full border border-white/15 px-1.5 py-[1px] text-[4px]">
          Apply
        </span>
      </div>
    </Phone>
